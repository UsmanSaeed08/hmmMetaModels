...
{	
	...
	iterate(Hmm<O> hmm, List<? extends List<? extends O>> sequences, ArrayList<Integer> silentStates)
	{			
		Hmm<O> nhmm;
		try {
			nhmm = hmm.clone();
		...
		for (List<? extends O> obsSeq : sequences) {	    
			ForwardBackwardCalculator fbc = 
				generateForwardBackwardCalculator(obsSeq, hmm, silentStates);			
		...
		/* pdfs computation */
		double[] density = new double[hmm.nbStates()];
		for (int i = 0; i < hmm.nbStates(); i++) {
...
//			System.out.println(i + ": " + sum);
		...
		nhmm.setDensity(density);
		return nhmm;
	}	
	
	public <O extends Observation> Hmm<O>
		iterate(Hmm<O> hmm, List<? extends List<? extends O>> sequences){
		return iterate(hmm, sequences, new ArrayList<Integer>()); 
	}	
	
	protected <O extends Observation> ForwardBackwardCalculator
	generateForwardBackwardCalculator(List<? extends O> sequence, Hmm<O> hmm, ArrayList<Integer> silentStates)
	{	
		return new ForwardBackwardCalculator(sequence, hmm, 
				EnumSet.allOf(ForwardBackwardCalculator.Computation.class), silentStates);
	}
	 ...
	 */
	public <O extends Observation> Hmm<O>
	learn(Hmm<O> initialHmm, List<? extends List<? extends O>> sequences, ArrayList<Integer> silentStates)
	{
		Hmm<O> hmm = initialHmm;
		
		for (int i = 0; i < nbIterations; i++){
			hmm = iterate(hmm, sequences, silentStates);
	...
	{
		return learn(initialHmm, sequences, new ArrayList<Integer>());
	}
