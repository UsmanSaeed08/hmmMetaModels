...
{	
	...
	protected ArrayList<Integer> silentStates;
	...
	public <O extends Observation>
	ForwardBackwardCalculator(List<? extends O> oseq,
			Hmm<O> hmm, EnumSet<Computation> flags, ArrayList<Integer> silentStates)
	{
		this.silentStates = silentStates;
		if (oseq.isEmpty())
			throw new IllegalArgumentException("Invalid empty sequence");
	 ...
	 */
	public <O extends Observation>
	ForwardBackwardCalculator(List<? extends O> oseq, Hmm<O> hmm, ArrayList<Integer> silentStates)
	{
	...
	{
		this(oseq, hmm, new ArrayList<Integer>());				
	}
	...
	{	
		...
		for (int t = 1; t < oseq.size(); t++) {
			...
			for (int i = 0; i < hmm.nbStates(); i++){
				if(!silentStates.contains(i)){
					computeAlphaStep(hmm, observation, t, i);
				}
			}
			for(int i = 0; i < hmm.nbStates(); i++){
				if(silentStates.contains(i)){
					computeAlphaStepSilent(hmm, observation, t, i);
				}
			}
		}
	}
	...
	protected <O extends Observation> void 
	computeAlphaStepSilent(Hmm<? super O> hmm, O o, int t, int j)
	{
		double sum = 0.;
	...
	computeBeta(Hmm<? super O> hmm, List<O> oseq)
	{
		...
		for (int t = oseq.size()-2; t >= 0; t--){
			...
			for(int i = 0; i < hmm.nbStates(); i++){
				if(silentStates.contains(i)){
					computeBetaStepSilent(hmm, oseq.get(t+1), t, i);
				}
			}		
		}
	}
	...
	protected <O extends Observation> void 
	computeBetaStepSilent(Hmm<? super O> hmm, O o, int t, int i)
	{
		double sum = 0.;
		
		for (int j = 0; j < hmm.nbStates(); j++)
			sum += beta[t][j] * hmm.getAij(i, j);			
