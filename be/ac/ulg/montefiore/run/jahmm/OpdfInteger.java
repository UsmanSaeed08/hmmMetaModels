...
{	
	...
	public void addPseudocounts(double nbSequences){
		int size = probabilities.length;		
		Pseudocount ps = new Pseudocount(probabilities);
		double[] X = ps.getPosteriorEstimates(nbSequences);
		double sumX = 0;
		...
		for(int i = 0; i < size; i++){
//			probabilities[i] = (c[i] + 1) / (sumC + size);
			probabilities[i] = X[i] / sumX;
		}
